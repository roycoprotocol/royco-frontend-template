...
import { DepositColumnDataElement, depositsColumns } from "./deposits-columns";

export const DepositsTable = React.forwardRef<
  HTMLTableElement,
  React.HTMLAttributes<HTMLTableElement> & {
    data: DepositColumnDataElement[];
    table: TableInstance<DepositColumnDataElement>;
  }
    ...
    <Table ref={ref} className={cn(className)} {...props}>
      <TableHeader className={cn("sticky top-0 z-10 [&_tr]:border-b-0")}>
        {table.getHeaderGroups().map((item) => (
          <TableRow key={item.id} className={cn("w-full hover:bg-transparent")}>
        ...
        {table.getRowModel().rows?.length === 0 ? (
          <TableRow>
            <TableCell
              ...
              className="h-24 w-full border-l border-r border-t border-_divider_ text-center"
            >
