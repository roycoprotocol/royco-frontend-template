...
type OfferDataType = {
  ...
  offer_id: number; // Offer ID of AP Offer
  ...
  fill_quantity: number; // Quantity of the offer that will be filled
...
async function getInvalidApOffers(offers: OfferDataType[]): Promise<string[]> {
  ...
  if (offers?.length) {
    const erc20Abi = ContractMap[11155111]["Erc20"].abi;

    for (let i = 0; i < offers.length; i++) {
      const offer = offers[i];

      console.log(offer.chain_id)
      ...
      const balanceData = await client.call({
        ...
        // @ts-ignore
        to: Number(offer.funding_vault) ? offer.funding_vault : inputTokenAddress // Use funding vault if provided, else token contract directly
      });

      const balance = Number(balanceData["data"]);

      if (balance < offer.fill_quantity) {
        invalidApOffers.push(offer.id);
        continue;
      }

      // @ts-ignore
      const allowedAddress = offer.market_type ? ContractMap[offer.chain_id]["VaultMarketHub"].address : ContractMap[offer.chain_id]["RecipeMarketHub"].address;
      const approvalData = await client.call({
        ...
        data: encodeFunctionData({
          ...
          // @ts-ignore
          args: [offer.creator, allowedAddress]
        }),
        // @ts-ignore
        to: Number(offer.funding_vault) ? offer.funding_vault : inputTokenAddress // Use funding vault if provided, else token contract directly
      });

      const allowance = Number(approvalData["data"]);

      if (allowance < offer.fill_quantity) {
        invalidApOffers.push(offer.id);
        continue;
      }
