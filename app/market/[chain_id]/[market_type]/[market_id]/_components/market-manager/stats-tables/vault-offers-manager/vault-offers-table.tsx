...
import {
  VaultOffersColumnDataElement,
  vaultOffersColumns,
} from "./vault-offers-columns";

export const VaultOffersTable = React.forwardRef<
  HTMLDivElement,
  React.HTMLAttributes<HTMLDivElement> & {
    data: VaultOffersColumnDataElement[];
    isLoading: boolean;
      ...
      <Table className={cn("")}>
        ...
        <TableBody className={cn("overflow-y-scroll bg-white")}>
          {table.getRowModel().rows?.length === 0 ? (
            <TableRow>
              <TableCell
                colSpan={vaultOffersColumns.length}
