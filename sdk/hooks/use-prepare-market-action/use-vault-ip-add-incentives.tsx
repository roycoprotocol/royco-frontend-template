  ...
  try {
    ...
    for (let i = 0; i < token_ids.length; i++) {
      ...
      if (enrichedMarket.incentive_ids?.includes(token_id)) {
        ...
        const existing_start_timestamp =
          start_timestamps?.[existing_incentive_index] ?? "0";
        const existing_end_timestamp =
          end_timestamps?.[existing_incentive_index] ?? "0";
        ...
        if (!BigNumber.from(existing_end_timestamp).eq(BigNumber.from("0"))) {
          // Check if an interval is already in progress
          if (
            BigNumber.from(current_timestamp).lte(
              BigNumber.from(existing_end_timestamp)
            )
          ) {
            throw new Error(
